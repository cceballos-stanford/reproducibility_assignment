...
data_modified <- read_excel("data/data_Exp1_modified.xlsx", sheet = "summary")
...
copilot comment: I think the pilot missed the fact that it is a repeated measures anova. So we need to include error terms in the "aov" function. If repeated measure if performed, the result of original paper is replicated. Also added results from post-hoc tests
...
# copilot: repeated measure aov function used  // testing - significant interaction between task and crowding condition
ANOVA_results2 <- aov(estimate ~ (factor(grasping_estimating)*factor(crowded_uncrowded)*factor(open_closed)*factor(cm)) + Error(factor(sub)/(factor(grasping_estimating)*factor(crowded_uncrowded)*factor(open_closed)*factor(cm))), data=ANOVA_tidy_data)
summary(ANOVA_results2[7])

```

Also performed analysis for post-hoc t-tests
```{r}
#post hoc t-tests
 
# no flacker

#estimate~ closed-loop t(9) = 7.23, p < .001 
t1=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="uncrowded" & open_closed=="closedloop" & grasping_estimating=='estimation'))

#estimate~ open-loop t(9) = 9.19, p < .001 
t2=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="uncrowded" & open_closed=="openloop" & grasping_estimating=='estimation'))


#grip~ closed-loop t(9) = 4.29, p = .002 
t3=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="uncrowded" & open_closed=="closedloop" & grasping_estimating=='grasping'))

#grip~ open-loop t(9) = 4.79, p = .001 
t4=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="uncrowded" & open_closed=="openloop" & grasping_estimating=='grasping'))

# flankers
#estimate~ closed-loop t(9) = 1.02, p = .334

t5=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="crowded" & open_closed=="closedloop" & grasping_estimating=='estimation'))


#estimate~ open-loop t(9) = 1.78, p = .108
t6=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="crowded" & open_closed=="openloop" & grasping_estimating=='estimation'))


#grip~ closed-loop t(9) = 4.21, p = .002 

t7=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="crowded" & open_closed=="closedloop" & grasping_estimating=='grasping'))

#grip~ open-loop t(9) = 3.392, p = .008 
t8=t.test(estimate ~ factor(cm),paired=TRUE, data = filter(ANOVA_tidy_data, crowded_uncrowded=="crowded" & open_closed=="openloop" & grasping_estimating=='grasping'))
...
# original paper result replicated F(1, 9) = 6.818, p = .028
reportObject <- reproCheck(reportedValue = '6.818', obtainedValue = 6.818, valueType = 'other')
reportObject <- reproCheck(reportedValue = '.028', obtainedValue = .0282, valueType = 'p')
...
reportObject <- reproCheck(reportedValue = '7.23', obtainedValue = as.numeric(t1[1]), valueType = 't')
reportObject <- reproCheck(reportedValue = '<.001', obtainedValue = as.numeric(t1[3]), valueType = 'p', eyeballCheck = TRUE)

#estimate~ open-loop t(9) = 9.19, p < .001 

reportObject <- reproCheck(reportedValue = '9.19', obtainedValue = as.numeric(t2[1]), valueType = 't') #minor rounding error found
reportObject <- reproCheck(reportedValue = '<.001', obtainedValue = as.numeric(t2[3]), valueType = 'p', eyeballCheck = TRUE)


#grip~ closed-loop t(9) = 4.29, p = .002 

reportObject <- reproCheck(reportedValue = '4.29', obtainedValue = as.numeric(t3[1]), valueType = 't')  
reportObject <- reproCheck(reportedValue = '.002', obtainedValue = as.numeric(t3[3]), valueType = 'p') 

#grip~ open-loop t(9) = 4.79, p = .001 

reportObject <- reproCheck(reportedValue = '4.79', obtainedValue = as.numeric(t4[1]), valueType = 't') #minor rounding error found
reportObject <- reproCheck(reportedValue = '.001', obtainedValue = as.numeric(t4[3]), valueType = 'p')  
...
# flankers
#estimate~ closed-loop t(9) = 1.02, p = .334
reportObject <- reproCheck(reportedValue = '1.02', obtainedValue = as.numeric(t5[1]), valueType = 't')
reportObject <- reproCheck(reportedValue = '.334', obtainedValue = as.numeric(t5[3]), valueType = 'p')  


#estimate~ open-loop t(9) = 1.78, p = .108
reportObject <- reproCheck(reportedValue = '1.78', obtainedValue = as.numeric(t6[1]), valueType = 't')
reportObject <- reproCheck(reportedValue = '.108', obtainedValue = as.numeric(t6[3]), valueType = 'p')  


#grip~ closed-loop t(9) = 4.21, p = .002 
reportObject <- reproCheck(reportedValue = '4.21', obtainedValue = as.numeric(t7[1]), valueType = 't')
reportObject <- reproCheck(reportedValue = '.002', obtainedValue = as.numeric(t7[3]), valueType = 'p')  


#grip~ open-loop t(9) = 3.392, p = .008 
reportObject <- reproCheck(reportedValue = '3.392', obtainedValue = as.numeric(t8[1]), valueType = 't')
reportObject <- reproCheck(reportedValue = '.008', obtainedValue = as.numeric(t8[3]), valueType = 'p')  
...
This reproducibility check was a success with a few minor rounding errors.
